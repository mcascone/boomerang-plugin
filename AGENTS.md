...
- **Linux GPG signing**: Implemented in CI. Linux releases are signed with GPG detached signatures (`.asc` files). Requires `GPG_PRIVATE_KEY` and `GPG_PASSPHRASE` secrets. See `docs/GPG_SIGNING.md` for details.
